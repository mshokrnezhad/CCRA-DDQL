...
class CPLEX:
    ...
    def define_model(self, mdl, z, g, g_sup, req_flw, res_flw, flw, p, req_pp, res_pp, req_d, res_d, d):
        ...
        # for not to assign first tier nodes
        # mdl.add_constraints(mdl.sum(g[i, j] for j in self.net_obj.get_first_tier_nodes()) == 0
    ...
    def add_priority_assignment_action_constraint(self, mdl, p, action):
        mdl.add_constraint(p[action["req_id"], action["pri_id"]] == 1)
    ...
    def solve(self, action, switch="srv_plc", assigned_nodes=[], assigned_priorities=[], assigned_paths=[]):
        ...
        if switch == "pri_asg":
            ...
            if len(action) != 0:
                mdl = self.add_priority_assignment_action_constraint(mdl, p, action)
        ...
        parsed_solution = {"g": {}, "p": {}, "req_flw": {}, "res_flw": {}, "info": "", "OF": 0, "done": True}
        ...
        if msg != "":
            ...
            try:
                ...
                raw_g = np.array([[1 if g[i, n].solution_value > 0 else 0
                                   for n in self.net_obj.NODES] for i in range(self.req_obj.NUM_REQUESTS)])
                ...
                for i in self.req_obj.REQUESTS:
                    parsed_g[i] = np.where(raw_g[i] == 1)[0][0]
                    priority = raw_p[i]
